...
export default function PrimaryButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className={`btn-touch w-full rounded-2xl px-5 text-[1.0625rem] font-semibold leading-snug transition focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-45 disabled:shadow-none ${variants[variant]}`}
    >
      {children}
    </button>
  );
